...
async def convert_video(
    ...
    try:
        ...
        def progress_callback(frame_num: int, total: int, progress: float):
        ...
        JOBS_STATUS[job_id]["result"] = {
            ...
            "fps": result["fps"],
            "mp4_url": f"/api/v1/download/{job_id}/video" if "mp4_path" in result else None,
            "txt_files_count": len(result.get("txt_files", [])),
            "png_files_count": len(result.get("png_files", [])),
        }
...
async def get_job_status(job_id: str):
    ...
    return JobStatus(
        ...
        eta_seconds=None,  # TODO: вычислить ETA
